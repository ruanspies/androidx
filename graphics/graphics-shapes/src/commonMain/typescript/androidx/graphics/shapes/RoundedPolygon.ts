...
import { Point, PointTransformer, TransformResult } from "./Point";
import { convex, debugLog, distance, DistanceEpsilon, FloatPi, radialToCartesian, square } from "./Utils";
import { Feature, Corner, Edge } from "./Features";
...
export class RoundedPolygon {
        ...
        for (let i = 0; i <= this.features.length; i++) {
            ...
            for (const cubic of featureCubics) {
                if (!cubic.zeroLength()) {
                    ...
                    if (lastCubic != null) {
                        const newPoints = [...lastCubic.points];
    ...
    static create(
        ...
        if (typeof arg1 === 'number') {
            ...
            const perVertexRounding = (arguments[5] as CornerRounding[]) ?? undefined;
