...
import { ArcConverter } from "./ArcConverter";
import { Point } from "./Point";
import { debugLog, DistanceEpsilon } from "./Utils";
import { detectFeatures } from "./FeatureDetector";
...
export class SvgPathParser {
    ...
    private parseCommand(command: Command) {
        if (command.isCloseCommand) {
            this.cubics.push(this.lineToCubic(this.position, this.cubics[0].points[0]));
            return;
        }
